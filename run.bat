...
for /L %%i in (1,1,32) do (
    ...
    :: Random date between Sep 3 and Dec 10
    set /a offset=!random! %% 127

    if !offset! LSS 20 (
        set month=1
        set /a day=12 + offset
    ) else if !offset! LSS 48 (
        set month=2
        set /a day=offset - 19
    ) else if !offset! LSS 79 (
        set month=3
        set /a day=offset - 47
    ) else if !offset! LSS 109 (
        set month=4
        set /a day=offset - 78
    ) else (
        set month=5
        set /a day=offset - 108
    )
    :: Random time
